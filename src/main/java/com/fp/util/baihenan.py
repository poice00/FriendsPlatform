...
#获取用户列表    
def getContent(html):
    reg=r'.+?<li.+?<div class="pic".+?\'(.+?)\'.+?src="(.+?)".+?<p class="intro">(.+?)</p>'
    contentre=re.compile(reg,re.DOTALL)
    contentlist=contentre.findall(html)
    return contentlist
#获取详细资料    
def getInfomation(html):
    reg=r'<div class="manPic">.+?<dl.*?>.+?<span>(.*?)ID.+?</span>(.+?)</dt>.+?</span>(.+?)</dt>.+?<div class="name">.+?<span>(.+?)</span>.+?<p>(.+?)<span class="cbd">/</span>(.+?)<span class="cbd">/</span>(.+?)<span class="cbd">/</span>(.+?)</p>.+?<div class="data">.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<div class="intr">(.+?)</div>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<h3>家庭状况.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>.+?</span>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<h3>择偶意向.+?<dd.*?>(.*?)<.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.*?)<.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>.+?<dd.*?>(.+?)</dd>'
    #
    contentre=re.compile(reg,re.DOTALL)
...
def saveToMySql(imgurl,infomationlist):
    ...
    cur=conn.cursor()
    sql = "insert into info(headImg,realName,realNameID,loveType,nickName,age,\
            height,education,city,house,car,animal,homeCity,salary,job,astroLogical,\
            Ismarraied,introduction,liveCity,bloodType,weigth,bodyType,nation,selfScore,religion,\
            liveStyle,children,IsSmoke,IsDrink,school,majorDirection,companyType,\
            companyIndustry,jobState,lauauge,familyRank,parentLive,fatherJob,\
...
def display(infomationlist):
    #for content in contentlist:
        ...
        for info in infomationlist:
            ...
            #for tag in taglist:
               #print '标签 : ',tag
            print '姓名: ',info[0].strip()
            print 'ID : ',info[1].strip()
            print '恋爱类型  : ',info[2].strip()
            print '昵称: ',info[3].strip()
            ...
            print '........个人及工作状况.......'
            print '住房: ',info[8].strip()
            print '购车: ',info[9].strip()
            print '属相 : ',info[10].strip()
            print '家乡: ',info[11].strip()
            print '月薪 : ',info[12].strip()
            print '职业 : ',info[13].strip()     
            print '星座 : ',info[14].strip()
            print '婚姻 : ',info[15].strip()
            print '交友宣言 : ',info[16].strip()
            print '居住地: ',info[17].strip()
            print '血型 : ',info[18].strip()
            print '体重 : ',info[19].strip()
            print '体型: ',info[20].strip()
            print '民族 : ',info[21].strip()
            print '自评 : ',info[22].strip()
            print '宗教: ',info[23].strip()
            print '生活作息: ',info[24].strip()
            print '有无子女: ',info[25].strip()
            print '是否吸烟: ',info[26].strip()
            print '是否喝酒: ',info[27].strip()
            print '毕业学校: ',info[28].strip()
            print '所学专业: ',info[29].strip()
            print '公司性质: ',info[30].strip()
            print '公司行业: ',info[31].strip()
            print '工作状况: ',info[32].strip()
            print '掌握语言: ',info[33].strip()
...
if __name__=="__main__":
    ...
    file_object = open('C:/Users/ssy/Desktop/bj/2.html')
    try:
